...
if (!submittedHtml) {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      style.id = '__download_font_fix';
      style.textContent = `
        .card-container * {
          -webkit-font-smoothing: antialiased;
        }
        .card-container .scrobbles-number,
        .card-container .scrobbles-mini-number,
        .card-container .username,
        .card-container .stat-column h3,
        .card-container .brand-rewind-date,
        .card-container .scrobbles-unit {
    ...
    // O card possui box-shadow para fora da própria caixa. Se usarmos
    // card.screenshot() diretamente, o Puppeteer corta essa parte da sombra.
    // Em vez de capturar a página inteira com page.screenshot(), criamos
    // temporariamente uma área transparente que contém SOMENTE o card,
    // com uma margem ao redor para a sombra.
    const captureInfo = await page.evaluate(() => {
      ...
      // Congela o tamanho visual antes de retirar o card do .card-column.
      // Isso evita que width:100% seja recalculado quando o card mudar
      // temporariamente de pai.
      const computed = getComputedStyle(card);
      const width = rect.width;
      const height = rect.height;

      const wrapper = document.createElement('div');
      wrapper.id = '__download_capture_wrapper';
      ...
      // O wrapper fica diretamente no body, fora de qualquer elemento que
      // pudesse cortar a sombra por overflow.
      ...
      // A página ao redor não participa da imagem. O PNG será transparente
      // fora do wrapper/card.
    ...
    if (!captureInfo) {
      return res.status(400).send('Card não encontrado.');
    }

    const wrapper = await page.$('#__download_capture_wrapper');
    if (!wrapper) {
      return res.status(400).send('Área de captura não encontrada.');
    }

    // element.screenshot() captura somente o wrapper + card + sombra.
    // Não usamos page.screenshot({ clip }), que anteriormente estava
    // transformando o download em uma captura da região da página.
