...
// Proxy para a API do Last.fm usando a chave configurada no servidor.
// A chave NÃO é enviada ao navegador; o card apenas recebe o resultado da API.
app.use('/api/lastfm', express.json({ limit: '100kb' }));
app.post('/api/lastfm', async (req, res) => {
  if (!LASTFM_API_KEY) {
    return res.status(503).json({ error: 10, message: 'LASTFM_API_KEY não configurada no servidor.' });
  }
  ...
  try {
    ...
    if (!method) {
      return res.status(400).json({ error: 6, message: 'Método do Last.fm não informado.' });
    }
    ...
    res.status(502).json({ error: 16, message: 'Não foi possível consultar o Last.fm.' });
  }
});
