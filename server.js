...
function getLastfmApiKey() {
  return String(process.env.LASTFM_API_KEY || '').trim();
}

// O navegador recebe apenas o estado da configuração, nunca o valor da chave.
app.get('/api/lastfm/status', (req, res) => {
  res.set('Cache-Control', 'no-store');
  res.json({ configured: !!getLastfmApiKey() });
});

// Proxy: todas as consultas usam exclusivamente LASTFM_API_KEY do Render.
app.use('/api/lastfm', express.json({ limit: '100kb' }));
app.post('/api/lastfm', async (req, res) => {
  const apiKey = getLastfmApiKey();

  if (!apiKey) {
    return res.status(503).json({
      error: 10,
      message: 'LASTFM_API_KEY não configurada no servidor.'
    });
  }
  ...
  try {
    ...
    if (!method) {
      return res.status(400).json({
        error: 6,
        message: 'Método do Last.fm não informado.'
      });
    }
    ...
    console.error('Erro ao consultar o Last.fm:', error);
    res.status(502).json({
      error: 16,
      message: 'Não foi possível consultar o Last.fm.'
    });
  }
});

const htmlFile = path.join(__dirname, 'LastfmCard.html');
const mobileHtmlFile = path.join(__dirname, 'LastfmCardMobile.html');
const fontsCssFile = path.join(__dirname, 'fonts.css');
...
function pickHtmlFile(req) {
  return isMobileRequest(req) ? mobileHtmlFile : htmlFile;
}

// O card usa os arquivos Inter locais. Estas rotas são necessárias porque
// o Express não expõe a pasta do projeto inteira como arquivos estáticos.
app.get('/fonts.css', (req, res) => {
  res.sendFile(fontsCssFile);
});
...
app.get('/fonts/:fontFile', (req, res) => {
  ...
  if (!allowed.has(req.params.fontFile)) {
    return res.status(404).end();
  }
  res.sendFile(path.join(fontsDir, req.params.fontFile));
});
...
app.get('/', (req, res) => {
  res.sendFile(pickHtmlFile(req));
});

app.get('/download', (req, res) => {
  const file = pickHtmlFile(req);
...
// Proxy simples de imagem: usado pela "Buscar paleta" no painel pra poder
// ler os pixels da foto do banner (Last.fm, TheAudioDB, Wikipédia, Unsplash)
// via <canvas> no navegador sem esbarrar em CORS — a imagem passa a vir da
// mesma origem do nosso servidor. Só repassa os bytes: não decodifica nem
// processa a imagem aqui (sem dependências novas).
const PROXY_IMAGE_TIMEOUT_MS = 10000;
const PROXY_IMAGE_MAX_BYTES = 15 * 1024 * 1024; // 15MB de segurança
...
app.get('/proxy-image', async (req, res) => {
  ...
  if (target.protocol !== 'http:' && target.protocol !== 'https:') {
    return res.status(400).json({ error: 'Só URLs http(s) são permitidas.' });
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), PROXY_IMAGE_TIMEOUT_MS);
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(502).json({ error: 'Não foi possível buscar a imagem de origem.' });
    }

    const contentType = response.headers.get('content-type') || '';
    if (!contentType.startsWith('image/')) {
      return res.status(415).json({ error: 'A URL informada não é uma imagem.' });
    }

    const contentLength = parseInt(response.headers.get('content-length') || '0', 10);
    if (contentLength && contentLength > PROXY_IMAGE_MAX_BYTES) {
    ...
    console.error('Erro ao repassar imagem:', err);
    if (!res.headersSent) res.status(502).json({ error: 'Não foi possível carregar a imagem.' });
  } finally {
    clearTimeout(timeout);
  }
});
...
app.post('/download-image', async (req, res) => {
  ...
  try {
    ...
    // Como page.setContent() não tem uma URL-base, adicionamos uma para que
    // o fonts.css e os arquivos Inter locais sejam carregados no PNG baixado.
    const baseUrl = `${req.protocol}://${req.get('host')}/`;
    const safeHtml = req.body
      .replace(/<head(\s[^>]*)?>/i, match =>
        `${match}<base href="${baseUrl}">`
      )
      .replace(/<link\s+rel=["']stylesheet["']\s+href=["']fonts\.css["']\s*\/?>/i,
